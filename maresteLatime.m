...
for i = 1:numarPixeliLatime
    ...
    disp(['Alegem drumul vertical numarul ' num2str(i) ...
    ...
    imgCopy = eliminaDrumVertical(imgCopy,drum); %voi folosi aceasta linie
    %de cod atunci cand voi modifica pe latime la Programare Dinamica si
    %Aleator; si pe inaltime la Greedy
    %imgCopy = inlocuiesteDrumVertical(imgCopy,drum);
end
...
for i = 1:numarPixeliLatime
    ...
    for idx = i : numarPixeliLatime
        for idxj = 1 : size(imgCopy, 1)
            matriceDrumuri(idxj,idx)=matriceDrumuri(idxj,idx)+1;
        end
    end
end
